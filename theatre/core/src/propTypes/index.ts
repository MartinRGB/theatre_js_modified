...
export interface FileSrcObject {
  src:string ,
  fileName:string
}

export interface PropTypeConfig_FileSrc extends ISimplePropType<'filesrc', object> {}
export const filesrc = (
  defaultValue: FileSrcObject,
  opts: {
    label?: string
    interpolate?: Interpolator<object>
  } = {},
...
function _ensureFileSrc(s: unknown): object | undefined | null {
  if(s != null){
...
export interface PropTypeConfig_NumberLiteral<T extends number>
  extends ISimplePropType<'numberLiteral', T> {
  valuesAndLabels: Record<T, number>
  as: 'menu' | 'switch'
}

export function numberLiteral<
  ValuesAndLabels extends {[key in string]: number},
>(
  /**
   * Default value (a string that equals one of the options)
   */
  defaultValue: Extract<keyof ValuesAndLabels, number>,
  /**
  ...
  opts: {
    as?: 'menu' | 'switch'
    label?: string
    interpolate?: Interpolator<Extract<keyof ValuesAndLabels, number>>
  } = {},
): PropTypeConfig_NumberLiteral<Extract<keyof ValuesAndLabels, number>> {
  return {
    type: 'numberLiteral',
    default: defaultValue,
    valuesAndLabels: {...valuesAndLabels},
    ...
    deserializeAndSanitize(
      json: unknown,
    ): undefined | Extract<keyof ValuesAndLabels, number> {
      if (typeof json !== 'number') return undefined
...
export type PropTypeConfig_AllSimples =
  ...
  | PropTypeConfig_NumberLiteral<$IntentionalAny>
  | PropTypeConfig_Rgba
  | PropTypeConfig_FileSrc
// ######################## File Props ########################
